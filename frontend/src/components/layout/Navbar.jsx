...
const Navbar = ({ onMenuClick }) => {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-card/95 px-2 sm:px-4 md:px-6 backdrop-blur-sm sticky top-0 z-40">
      {/* Left */}
      <div className="flex items-center gap-1 sm:gap-3 flex-1 xl:flex-none overflow-hidden">
        <button onClick={onMenuClick} className="xl:hidden p-1.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted/50 transition-colors active:scale-95 shrink-0" aria-label="Open menu">
          <Menu className="w-5 h-5" />
        </button>
        <Link to={getRoleDashboardPath(user?.role)} className="xl:hidden flex items-center cursor-pointer ml-1 shrink-0">
      ...
      <div className="flex items-center gap-1 sm:gap-3 md:gap-4 relative shrink-0">
        ...
        <div className="w-[1px] h-5 bg-border hidden xl:block shrink-0" />
        ...
        <button 
          className="xl:hidden w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-white font-bold text-xs sm:text-sm shrink-0 active:scale-95 transition-transform ml-1" 
        ...
        <button
          ...
          title="Logout"
          className="hidden xl:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-border text-xs font-medium text-muted-foreground hover:bg-muted/50 transition-colors bg-transparent cursor-pointer shrink-0"
        >
